...
function skipUnavailableSong() { const failedId = currentVideoId(); if (failedId) failedVideoIds.add(failedId); for (let offset = 1; offset <= songs.length; offset++) { const nextIndex = (index + offset) % songs.length; if (!failedVideoIds.has(songs[nextIndex][4])) { change(nextIndex, true); return; } } playing = false; clearInterval(timer); render(); }
...
$('#info').addEventListener('click', () => $('#about').scrollIntoView({ behavior: 'smooth' }));
