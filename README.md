...
3. Deploy. The `/api/fresh-hits` Vercel Function combines the fixed featured playlist `PLwyqDgjhF4Qpq6kslMcQ8NR6RHb-Z4Tu-` with a weekly list of embeddable Bhojpuri music, excludes devotional/religious terms, and removes duplicate videos before the browser receives it. The browser keeps the hardcoded fallback songs first and appends these YouTube songs, so playback continues from the last hardcoded song into the YouTube list.
